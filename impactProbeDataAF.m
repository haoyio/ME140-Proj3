function [res] = impactProbeDataAF(mdot_air, mdot_fuel, A, Tm, Po, RF)
...
while (true) % Loop to find correct To
    ...
    MFP = ((mdot_air+mdot_fuel) / A) * (sqrt(R*To) / Po); % Equation 1
    ...
    while (true)
        [~, ~, k] = specHeatC123H222(mdot_air/mdot_fuel, T); % Calculate k based on temp
